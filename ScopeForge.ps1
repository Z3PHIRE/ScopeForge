...
function Export-ReconReport {
    ...
    if ($triageData) {
        ...
        if (@($triageShortlist).Count -eq 0) {
            ...
            if ($baselineReachableTargets.Count -gt 0) {
                $shortlistLines.Add('## Baseline Reachable Targets') | Out-Null
                $shortlistLines.Add('') | Out-Null
                $shortlistLines.Add('- No scored shortlist entries were generated; these retained reachable targets are provided for first-pass manual review.') | Out-Null
                $shortlistLines.Add('') | Out-Null
                foreach ($item in $baselineReachableTargets) {
                    ...
                    if ($itemContentType) {
                        $shortlistLines.Add(('- Content-Type: {0}' -f $itemContentType)) | Out-Null
                    }
                    if ($itemWebServer) {
                        $shortlistLines.Add(('- Web server: {0}' -f $itemWebServer)) | Out-Null
                    }
                    if ($item.Technologies -and $item.Technologies.Count -gt 0) {
    ...
    $deadStatusCodes = @(0, 404, 410)
    $reachableLiveTargets = @($LiveTargets | Where-Object { [int]$_.StatusCode -notin $deadStatusCodes })
    $deadOrUnstableTargets = @($LiveTargets | Where-Object { [int]$_.StatusCode -in $deadStatusCodes })
    $reachableLiveTargetCount = @($reachableLiveTargets).Count
    $deadOrUnstableTargetCount = @($deadOrUnstableTargets).Count
    ...
    $protectedTargetCount = @($reachableLiveTargets | Where-Object { $_.StatusCode -in 401, 403 }).Count
    ...
    $shortlistSectionSubtitle = 'Top 15 URLs to investigate first, kept inline as cards for fast review.'
    $shortlistCardsHtml = @(
    ...
    if ([string]::IsNullOrWhiteSpace($shortlistCardsHtml) -and @($shortlistFallbackTargets).Count -gt 0) {
        $shortlistSectionSubtitle = 'No scored shortlist entries were generated; retained reachable targets are shown here for first-pass manual review.'
        $shortlistCardsHtml = @(
            foreach ($item in $shortlistFallbackTargets) {
    ...
    $kpiCardsHtml = @(
        ...
        "<div class=""kpi-card""><div class=""kpi-icon tone-info""><svg viewBox=""0 0 24 24"" fill=""none"" stroke=""currentColor"" stroke-width=""1.8""><circle cx=""12"" cy=""12"" r=""8""/><circle cx=""12"" cy=""12"" r=""3""/><path d=""M12 2v3M12 19v3M2 12h3M19 12h3""/></svg></div><div><div class=""kpi-label"">HTTP Targets</div><div class=""kpi-value"">$(ConvertTo-HtmlSafe ([string]$Summary.LiveTargetCount))</div></div></div>"
...
<details open id="section-live" class="report-section"><summary><span>Live Targets</span><small>Reachable HTTP(S) targets retained after in-scope validation. $reachableLiveTargetCount item(s).</small></summary><div class="section-body"><div class="table-scroll"><table data-filter-table="true" data-sort-table="true"><thead><tr><th><button type="button">Host</button></th><th><button type="button">URL</button></th><th><button type="button">Status</button></th><th><button type="button">Title</button></th><th><button type="button">Technologies</button></th></tr></thead><tbody>$liveRows</tbody></table></div></div></details>
<details open id="section-dead" class="report-section"><summary><span>Dead / Unstable Targets</span><small>Dead or unstable HTTP targets preserved for evidence and noise separation. $deadOrUnstableTargetCount item(s).</small></summary><div class="section-body"><div class="table-scroll"><table data-filter-table="true" data-sort-table="true"><thead><tr><th><button type="button">Host</button></th><th><button type="button">URL</button></th><th><button type="button">Status</button></th><th><button type="button">Title</button></th><th><button type="button">Technologies</button></th></tr></thead><tbody>$deadRows</tbody></table></div></div></details>
